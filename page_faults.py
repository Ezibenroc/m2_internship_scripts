...
def measure_page_faults(shared, size, mem_access):
    ...
    assert major == 0
    return sys_time, usr_time, minor, cpu_utilization
    ...
    with open(file_name, 'w') as f:
        ...
        for exp in range(nb_exp):
            ...
            for shared, mem_access in experiments:
                sys_time, usr_time, nb_page_faults, cpu_utilization = measure_page_faults(shared, size, mem_access)
                csv_writer.writerow((shared, size, mem_access, sys_time, usr_time, nb_page_faults, cpu_utilization))
