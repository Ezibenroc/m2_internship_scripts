...
for i in $*; do (
	...
	run_command $i 'cd hpl* && sed -ri "s|TOPdir\s*=.+|TOPdir="`pwd`"|g" Make.SMPI && make startup arch=SMPI && make SMPI_OPTS="-DSMPI_OPTIMIZATION" arch=SMPI' # use only the Phi coefficients
