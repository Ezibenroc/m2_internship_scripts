...
for i in $*; do (
	...
	run_command $i 'cd hpl* && sed -ri "s|TOPdir\s*=.+|TOPdir="`pwd`"|g" Make.SMPI && make startup arch=SMPI && make SMPI_OPTS="-DSMPI_OPTIMIZATION -DSMPI_DGEMM_COEFFICIENT=1.981e-12 -DSMPI_DTRSM_COEFFICIENT=1.954e-12" arch=SMPI' # use only the Phi coefficients
